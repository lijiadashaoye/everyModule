...
export class TwoChild1Component implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.user = this.fb.group({
      ...
      name: [
        "",
        [Validators.required, Validators.minLength(2), this.nameMatcher]
      ], // 对单个表单进行验证
  ...
  nameMatcher(c: FormControl) {
    // 自定义验证函数,此处是对整表单中的单个control进行验证
    if (c.value > 10) {
      return {
        from: "单个表单进行验证"
      };
  ...
  otherTableFun() {
    // 获取表格的另一种方法，angular中尽量不要用原生 DOM 操作方法
  ...
  makePositin(e, mouseX, mouseY) {
    ...
    target.onmousemove = ev => {
      if (!this.offw) {
        this.offw = target.offsetWidth;
      }
      if (!this.offh) {
        this.offh = target.offsetHeight;
      }
      ...
      if (
        this.offLeft >= 0 &&
        this.offTop >= 0 &&
        this.offLeft <= offRight &&
        this.offTop <= offBottom
      ) {
        this.rd.setAttribute(
          target,
          "style",
        ...
        let kk = `clip:rect(${target.offsetTop}px ${e.target.offsetLeft +
        ...
        if (this.offLeft > offRight) {
          this.offLeft = offRight;
        }
        if (this.offTop > offBottom) {
          this.offTop = offBottom;
        }
        this.rd.setAttribute(
          target,
  ...
  makeSize(e, mouseX, mouseY) {
    if (e["target"].parentNode.id == "main") {
      ...
      target.onmousemove = ev => {
        ...
        let offRight =
          e.target.parentNode.parentNode.offsetWidth + e.target.offsetWidth / 2; // 右侧的距离
        let offBottom =
