...
export class TwoChild3Component implements OnInit {
  ...
  seeScan() {
    ...
    // 计算5秒内发生的点击次数
    var clicksInFiveSeconds = Observable.fromEvent(document, "click").takeUntil(
  ...
  merges() {
    ...
    let merges1$ = Observable.fromEvent(merges1, "keyup").pluck(
      "target",
      "value"
    );
    let merges2$ = Observable.fromEvent(merges2, "keyup").pluck(
      "target",
      "value"
    );

    Observable.merge(merges1$, merges2$).subscribe(
      val => (this.mergeData = val)
    );
  ...
  forkJoin_concat() {
    ...
    Observable.concat(one, two).subscribe(value => console.log(value));

    // 将多个 Observables 的最后一个值，组成对应顺序的数组发出来。
    // forkJoin 是 Rx 版本的 Promise.all()，即表示等到所有的 Observable 都完成后，才一次性返回值。
    Observable.forkJoin(one, two).subscribe(value => console.log(value));
  }
  everyObservable() {
    this.merges();
    ...
    let pluck1$ = Observable.fromEvent(pluck1, "keyup")
      .pluck("target", "value")
      .subscribe(val => (this.Observable1 = val));

    let map1 = this.el.nativeElement.querySelector(".map");
    let map1$ = Observable.fromEvent(map1, "keyup")
    ...
    let combineLatest1$ = Observable.fromEvent(combineLatest1, "keyup").map(
      ev => ev["target"].value
    );
    let combineLatest2$ = Observable.fromEvent(combineLatest2, "keyup").map(
      ev => ev["target"].value
    );
    ...
    this.newSubject.subscribe(x => console.log(x), error => console.log(error));
